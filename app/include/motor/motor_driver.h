...
typedef int (*motor_set_effort_t)(const struct device *dev, int32_t effort_q31);
...
__subsystem struct motor_driver_api {
     ...
     * @param effort_q31 Desired normalized Q31 effort in range [INT32_MIN, INT32_MAX].
...
static inline int motor_set_effort(const struct device *dev, int32_t effort_q31)
{
    ...
    if (!motor_effort_q31_is_valid(effort_q31)) {
        return -EINVAL;
    }
