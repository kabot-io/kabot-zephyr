...
{
	LOG_INF("Publishing control message: left=%.3f, right=%.3f", (double)msg->effort.state.x,
...
bool control_channel_validator(const void *msg, size_t msg_size)
{
	...
	if (!valid) {
		LOG_WRN("Control channel validator failed: left=%.3f, right=%.3f, msg_size=%zu",
			(double)left, (double)right, msg_size);
	}

	return valid;
}

ZBUS_CHAN_DEFINE(control_channel, Control, control_channel_validator, NULL,
