...
BUILD_ASSERT(DT_HAS_ALIAS(kabot_distance),
	     "No devicetree alias 'kabot-distance' found for distance publisher");
BUILD_ASSERT(DT_NODE_HAS_STATUS_OKAY(DT_ALIAS(kabot_distance)),
	     "Distance devicetree node is not enabled");
...
void distance_publisher_task(void)
{
	...
	LOG_INF("Distance publisher active: %d ms (%s)", CONFIG_KABOT_STATE_DISTANCE_PERIOD_MS,
		tof->name);
...
K_THREAD_DEFINE(distance_publisher_task_id, CONFIG_KABOT_DISTANCE_PUBLISHER_STACK_SIZE,
		distance_publisher_task, NULL, NULL, NULL, 4, 0, 0);
