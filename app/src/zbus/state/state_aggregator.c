...
static bool should_replace_field(bool incoming_has_header, uint64_t incoming_stamp,
				 bool has_current, bool current_has_header, uint64_t current_stamp)
{
...
static void merge_state_if_newer(State *combined, const State *incoming)
{
	if (incoming->has_effort &&
	    should_replace_field(incoming->effort.has_header, incoming->effort.header.stamp,
				 combined->has_effort, combined->effort.has_header,
	...
	if (incoming->has_angular_velocity &&
	    should_replace_field(
		    incoming->angular_velocity.has_header, incoming->angular_velocity.header.stamp,
		    combined->has_angular_velocity, combined->angular_velocity.has_header,
	...
	if (incoming->has_magnetic_field &&
	    should_replace_field(incoming->magnetic_field.has_header,
				 ...
				 combined->has_magnetic_field, combined->magnetic_field.has_header,
	...
	if (incoming->has_distance &&
	    should_replace_field(incoming->distance.has_header, incoming->distance.header.stamp,
				 combined->has_distance, combined->distance.has_header,
	...
	if (incoming->has_light_left &&
	    should_replace_field(incoming->light_left.has_header, incoming->light_left.header.stamp,
				 combined->has_light_left, combined->light_left.has_header,
	...
	if (incoming->has_light_right &&
	    should_replace_field(incoming->light_right.has_header,
				 incoming->light_right.header.stamp, combined->has_light_right,
	...
	if (incoming->has_current_left &&
	    should_replace_field(incoming->current_left.has_header,
				 incoming->current_left.header.stamp, combined->has_current_left,
	...
	if (incoming->has_bus_voltage_left &&
	    should_replace_field(
		    incoming->bus_voltage_left.has_header, incoming->bus_voltage_left.header.stamp,
		    combined->has_bus_voltage_left, combined->bus_voltage_left.has_header,
	...
	if (incoming->has_power_left &&
	    should_replace_field(incoming->power_left.has_header, incoming->power_left.header.stamp,
				 combined->has_power_left, combined->power_left.has_header,
	...
	if (incoming->has_current_right &&
	    should_replace_field(incoming->current_right.has_header,
				 incoming->current_right.header.stamp, combined->has_current_right,
	...
	if (incoming->has_power_right &&
	    should_replace_field(incoming->power_right.has_header,
				 incoming->power_right.header.stamp, combined->has_power_right,
	...
	if (incoming->has_current_supply &&
	    should_replace_field(incoming->current_supply.has_header,
				 ...
				 combined->has_current_supply, combined->current_supply.has_header,
	...
	if (incoming->has_power_supply &&
	    should_replace_field(incoming->power_supply.has_header,
				 incoming->power_supply.header.stamp, combined->has_power_supply,
