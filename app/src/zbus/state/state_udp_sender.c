...
static bool wifi_ready_for_state_egress(void)
{
	...
	bool ipv4_ready =
		wifi_connected && (net_if_ipv4_get_global_addr(iface, NET_ADDR_PREFERRED) != NULL);
...
static int apply_state_target(struct sockaddr_in *dest, char *active_ip, size_t active_ip_len,
...
void state_udp_sender_task(void)
{
	...
	while (!zbus_sub_wait(&state_udp_sender, &chan, K_FOREVER)) {
		...
		if (!robot_settings_is_claimed()) {
			...
			if ((last_unclaimed_log_ms == 0) ||
			    ((now_ms - last_unclaimed_log_ms) >= 5000)) {
				LOG_INF("State UDP waiting for claim; dropping egress until "
					"claim=true Bonjour arrives");
		...
		State state;
		if (zbus_chan_read(&state_egress_channel, &state, K_MSEC(ZBUS_READ_TIMEOUT_MS)) !=
		    0) {
		...
		if (!wifi_ready_for_state_egress()) {
			...
			if (!network_ready) {
				if (last_wait_log_ms == 0 || (now_ms - last_wait_log_ms) >= 5000) {
					LOG_INF("State UDP dropping messages: waiting for Wi-Fi/IP "
						"(%lld ms)",
						waited_ms);
					last_wait_log_ms = now_ms;
				}
			} else {
				LOG_WRN("State UDP network no longer ready; dropping until "
					"Wi-Fi/IP recovers");
				network_ready = false;
		...
		pb_ostream_t stream =
			pb_ostream_from_buffer(state_encode_buf, sizeof(state_encode_buf));
...
K_THREAD_DEFINE(state_udp_sender_task_id, CONFIG_MAIN_STACK_SIZE, state_udp_sender_task, NULL, NULL,
		NULL, 4, 0, 0);
