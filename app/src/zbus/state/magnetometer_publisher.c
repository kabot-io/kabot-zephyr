...
BUILD_ASSERT(DT_HAS_ALIAS(kabot_mag),
	     "No devicetree alias 'kabot-mag' found for magnetometer publisher");
BUILD_ASSERT(DT_NODE_HAS_STATUS_OKAY(DT_ALIAS(kabot_mag)),
	     "Magnetometer devicetree node is not enabled");
...
void magnetometer_publisher_task(void)
{
	...
	LOG_INF("Magnetometer publisher active: %d ms (%s)", CONFIG_KABOT_STATE_MAG_PERIOD_MS,
		mag->name);
...
K_THREAD_DEFINE(magnetometer_publisher_task_id, CONFIG_KABOT_MAGNETOMETER_PUBLISHER_STACK_SIZE,
		magnetometer_publisher_task, NULL, NULL, NULL, 4, 0, 0);
