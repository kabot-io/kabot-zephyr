...
void light_publisher_task(void)
{
	...
	LOG_INF("Light publisher active: %d ms (%s, %s)", CONFIG_KABOT_STATE_LIGHT_PERIOD_MS,
		light_left->name, light_right->name);
	...
	while (true) {
		...
		if (left_valid) {
			...
			set_header_frame_id(&state.light_left.header,
					    CONFIG_KABOT_STATE_LIGHT_LEFT_FRAME_ID);
		...
		if (right_valid) {
			...
			set_header_frame_id(&state.light_right.header,
					    CONFIG_KABOT_STATE_LIGHT_RIGHT_FRAME_ID);
...
K_THREAD_DEFINE(light_publisher_task_id, CONFIG_KABOT_LIGHT_PUBLISHER_STACK_SIZE,
		light_publisher_task, NULL, NULL, NULL, 4, 0, 0);
