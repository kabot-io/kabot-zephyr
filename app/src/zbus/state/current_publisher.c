...
static bool read_ina219_triplet(const struct device *dev, const char *name, float *current,
				float *bus_voltage, float *power)
{
...
void current_publisher_task(void)
{
	...
	LOG_INF("Current publisher active: %d ms (%s, %s, %s)",
		CONFIG_KABOT_STATE_CURRENT_PERIOD_MS, left->name, right->name, supply->name);
	...
	while (true) {
		...
		bool left_valid = read_ina219_triplet(left, "left", &left_current,
						      &left_bus_voltage, &left_power);
		bool right_valid = read_ina219_triplet(right, "right", &right_current,
						       &right_bus_voltage, &right_power);
		bool supply_valid = read_ina219_triplet(supply, "supply", &supply_current,
							&supply_bus_voltage, &supply_power);
...
K_THREAD_DEFINE(current_publisher_task_id, CONFIG_KABOT_CURRENT_PUBLISHER_STACK_SIZE,
		current_publisher_task, NULL, NULL, NULL, 4, 0, 0);
