...
static void handle_bonjour_packet(int fd,
                                  ...
                                  size_t len)
{
    ...
    LOG_INF("Bonjour RX from %s:%u hmi_port=%u claim=%s release=%s",
            sender_ip,
            (unsigned int)sender_port,
            (unsigned int)hmi_port,
            msg.claim ? "true" : "false",
    ...
    if (msg.release) {
        ...
        LOG_INF("Bonjour CLAIM takeover: %s:%u -> %s:%u",
                old_ip,
                (unsigned int)old_port,
                new_ip,
        ...
        LOG_INF("Bonjour DISCOVER only from %s:%u (no takeover)",
                sender_ip,
                (unsigned int)sender_port);
    }
    ...
    LOG_INF("Bonjour response TX mode=%s to %s:%u serial=%s name=%s claimed=%s claimed_by=%s control_port=%u fw=%s bytes=%d",
            mode,
            sender_ip,
            (unsigned int)sender_port,
            serial,
            human_name,
            response.is_claimed ? "true" : "false",
            claimed_by_ip[0] ? claimed_by_ip : "",
            (unsigned int)response.control_port,
            CONFIG_KABOT_FIRMWARE_VERSION,
            (int)sent);
}
