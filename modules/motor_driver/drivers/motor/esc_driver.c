				    ...
				    uint32_t forward_pulse)
{
	if (effort <= 0.0f) {
		...
		return (uint32_t)lroundf((float)reverse_pulse +
					 ((float)stop_pulse - (float)reverse_pulse) * t);
	}
	...
	return (uint32_t)lroundf((float)stop_pulse +
				 ((float)forward_pulse - (float)stop_pulse) * t);
}
